...
class ClassReader {
        ...
        this.attributes.forEach(function (a) {
            if (a.info.type === ATTRIBUTE_TYPES.InnerClasses) {
                a.info.classes.forEach(function (c) {
                    try {
                        if (c.innerClassInfoIndex > 0) {
                            classes.push(cp.getValue(cp.getValue(c.innerClassInfoIndex).nameIndex).string);
                        }
                        if(c.outerClassInfoIndex > 0) {
                            classes.push(cp.getValue(cp.getValue(c.outerClassInfoIndex).nameIndex).string);
                        }
                    } catch (ex) {
                        console.error(ex);
                    }
    ...
    getFieldDescriptor(field) {
        ...
        return {accessFlagsString: this.decodeAccessFlags(field.accessFlags), name, descriptor, type: this.decodeDescriptorType(descriptor), signature: this.decodeSignature(descriptor).returnType, accessFlags: field.accessFlags, text: this.createFieldSignature(name, descriptor, field.accessFlags)};
    }
    ...
    getMethodDescriptor(method) {
        ...
        return {
            ...
            longText: this.createMethodSignature(`${this.getFriendlyClassName()}.${name}`, descriptor, method.accessFlags)};
    }
    ...
    readAttributes(attributeNameIndex, bytes) {
        ...
        let item = this.constantPool.getValue(attributeNameIndex);
    ...
    read() {
        ...
        this.interfaces = [];
        let interfacesCount = this.reader.readUShort();
        for (let i = 0; i < interfacesCount; i++) {
            let index = this.reader.readUShort();
            if (index != 0) {
        ...
        this.fields = [];
        let fieldsCount = this.reader.readUShort();
        for (let i = 0; i < fieldsCount; i++) {
            let accessFlags = this.reader.readUShort();
            let nameIndex = this.reader.readUShort();
            let descriptorIndex = this.reader.readUShort();
            let attributesCount = this.reader.readUShort();
            let fieldInfo = {
                accessFlags,
                nameIndex,
                descriptorIndex,
            ...
            for (let j = 0; j < attributesCount; j++) {
                let attributeNameIndex = this.reader.readUShort();
                let attributeLength = this.reader.readUInt();
                let info = this.reader.readBytes(attributeLength);
        ...
        this.methods = [];
        let methodsCount = this.reader.readUShort();
        for (let i = 0; i < methodsCount; i++) {
            let accessFlags = this.reader.readUShort();
            let nameIndex = this.reader.readUShort();
            let signatureIndex = this.reader.readUShort();
            let attributesCount = this.reader.readUShort();
            let methodInfo = {
                accessFlags,
                nameIndex,
            ...
            for (let j = 0; j < attributesCount; j++) {
                let attributeNameIndex = this.reader.readUShort();
                let attributeLength = this.reader.readUInt();
                let info = this.readAttributes(attributeNameIndex, this.reader.readBytes(attributeLength));
                let attribute = {
                    attributeNameIndex,
                    attributeLength,
                    info
