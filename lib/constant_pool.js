...
class ConstantPool {
    ...
    getValue(idx) {
        if (idx < 0 || idx >= this.cp.length) {
            throw new Error('ConstantPool: Access outside of the boundaries');
        }
        return this.cp[idx];
    }
    ...
    resolve(idx){
        ...
        switch(base.tag){
            case TAG.CONSTANT_Class:
                ...
                result.text = result.className = result.value = this.resolve(base.nameIndex).value;
                break;
            ...
            case TAG.CONSTANT_Methodref:
                ...
                result.className = this.resolve(base.classIndex).className;
                nameResolve = this.resolve(base.nameAndTypeIndex);
                result.methodName = ["<init>", "<clinit>"].includes(nameResolve.name) ? result.className.substring(result.className.lastIndexOf("/") + 1) : nameResolve.name;
                result.returnType = nameResolve.returnType;
                result.parameterTypes = nameResolve.parameterTypes;
                result.text = `${result.returnType} ${this.ca.getFriendlyClassName(result.className)}.${result.methodName}(${result.parameterTypes ? result.parameterTypes.join(', ') : ""})`;
                break;
            case TAG.CONSTANT_InterfaceMethodref:
                result.tagName = 'InterfaceMethodref';
                result.className = this.resolve(base.classIndex).className;
                nameResolve = this.resolve(base.nameAndTypeIndex);
                result.methodName = result.interfaeMethodName = nameResolve.name;
                result.methodType = result.interfaeMethodType = nameResolve.type;
                result.returnType = nameResolve.returnType;
                result.parameterTypes = nameResolve.parameterTypes;
            ...
            case TAG.CONSTANT_String:
                ...
                result.text = result.value = base.string ? `"${base.string}"` : `"${this.resolve(base.stringIndex).value}"`;
                break;
            ...
            case TAG.CONSTANT_NameAndType:
                ...
                result.parameterTypes = signature.parameterTypes;
